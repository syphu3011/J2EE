...
const hoadon = require("../schemas/hoadon");
module.exports = {
  Mutation: {
      ...
      try {
        ...
        for (const sp of sanpham) {
          ...
          await hangtrongkho.update({
            soluong: hangtrongkho.soluong - sp.soluong,
          });
        }
    ...
    async xacnhanhoachuyhoadon(root, args, context) {
      ...
      try {
        ...
        var mainOptions = {
          ...
          to: hoadon.email,
          subject:
            matrangthai == 2 ? "Đơn hàng được xác nhận!" : "Đơn hàng bị hủy!",
          text:
            "Đơn hàng có mã " +
            hoadon.ma +
            (matrangthai == 2 ? " đã được cửa hàng xác nhận!" : " đã bị hủy!"),
        };
    ...
    tongtien: async (hoadon) => {
      try {
        ...
        if (result && result.length > 0) {
          ...
          return 0; // Trả về 0 nếu không có dữ liệu hoặc kết quả trả về không hợp lệ
        }
      } catch (error) {
        console.error(
          "Đã xảy ra lỗi khi tính tổng tiền chi tiết hóa đơn:",
          error
        );
