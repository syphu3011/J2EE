...
const DeliveryInform = () => {
  ...
  // Tính tổng itemTotal
  const totalItemTotal = items.reduce(
    (accumulator, item) => accumulator + item.itemTotal,
    0
  );
  ...
  const convertToNonAccentVietnamese = (string) => {
    var accentVietnamese =
      "àáảãạăằắẳẵặâầấẩẫậèéẻẽẹêềếểễệìíỉĩịòóỏõọôồốổỗộơờớởỡợùúủũụưừứửữựỳýỷỹỵđÀÁẢÃẠĂẰẮẲẴẶÂẦẤẨẪẬÈÉẺẼẸÊỀẾỂỄỆÌÍỈĨỊÒÓỎÕỌÔỒỐỔỖỘƠỜỚỞỠỢÙÚỦŨỤƯỪỨỬỮỰỲÝỶỸỴĐ";
    var nonAccentVietnamese =
      "aaaaaaaaaaaaaaaaaeeeeeeeeeeeiiiiiooooooooooooooooouuuuuuuuuuuyyyyydAAAAAAAAAAAAAAAAAEEEEEEEEEEEIIIIIOOOOOOOOOOOOOOOOOUUUUUUUUUUUYYYYYYD";
  ...
  const generateAndDownloadPDF = async (invoiceDetails, productList) => {
    ...
    doc.text(
      "Recipient's Name: " +
        convertToNonAccentVietnamese(invoiceDetails.fullname),
      10,
      20
    );
    ...
    doc.text(
      "Delivery Address: " +
        convertToNonAccentVietnamese(invoiceDetails.address),
      10,
      50
    );
    ...
    const totalTextX =
      doc.internal.pageSize.getWidth() - totalTextWidth - totalAmountWidth - 15;
    const totalAmountX =
      doc.internal.pageSize.getWidth() - totalAmountWidth - 15;
  ...
  const handleSubmitDeliver = async (invoiceDetails) => {
    ...
    async function getIdFromName(name, colorname, sizename) {
      ...
      if (data) {
        const matchingMathang = data.mathang.find(
          (e) => e.mau.ten === colorname && e.kichco.ten === sizename
        );
        if (matchingMathang) {
          return {
      ...
      if (soluong > sanpham.soluong) {
        Modal.error({
          content:
            ...
            " chỉ còn " +
            sanpham.soluong +
            ". Quý khách vui lòng sửa số lượng và đặt lại!",
        });
      }
                ...
                >
                  ...
                  >
                    <Input
                      ...
                      size="large"
                      value={isLoggedIn ? users[0].email : ""}
                    />
                  </Form.Item>
                  <Form.Item
                    name="address"
                  ...
                  <Form.Item>
                    <Input
                      type="submit"
                      value="Xác nhận"
                      className="btn-btnUpdateInformation"
                      size="large"
                    />
                  </Form.Item>
                </Form>
          ...
          <Col flex="2">
            <div className="list-product-checkout">
              <List>
                {items.slice(minValue, maxValue).map((item) => (
                  <List.Item key={item.id}>
                    <List.Item.Meta
                      ...
                      description={
                        <div>
                          Màu: {item.color} - Kích thước: {item.size} <br />
                          Số lượng: {item.quantity}{" "}
                          <p style={{ float: "right" }}>
                            Tổng cộng: {item.itemTotal.toLocaleString()}
                          </p>{" "}
                          <br />
                ...
                >
                  Thành tiền : {totalItemTotal.toLocaleString()} VND
                </p>
              </List>
              <Pagination
                defaultCurrent={1}
